...
/**
 * Embedding config. Defaults are hardcoded so .env stays optional;
 * EMBEDDING_DIMS must match the Space.embedding column (vector(768)).
 */
...
/** Lazily build the Gemini embedding model; returns null when there is no API key. */
...
/** Resolve space amenities to human-readable labels for embedding text. */
...
/**
 * Build the canonical text document representing a space for embedding.
 * Tolerates sparse input: omit empty sections and never throw.
 */
...
/** Retry helper with exponential backoff on rate-limit errors (429). */
...
/** Embed a stored document (space listing). Returns number[] or null on failure / missing key. */
export function embedDocument(text) {
  return embedWithTaskType(text, TaskType.RETRIEVAL_DOCUMENT);
}

/** Embed a search query. Returns number[] or null on failure / missing key. */
export function embedQuery(text) {
  return embedWithTaskType(text, TaskType.RETRIEVAL_QUERY);
}

/**
 * Embed many documents in batches with retry on 429. Returns an array aligned with
