...
export const Hero = () => {
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pt-20 md:pt-24 pb-10 md:pb-12 px-4">
      ...
      <div className="relative z-10 w-full max-w-4xl px-2 text-center sm:px-6 lg:px-8">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
        >
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white mb-3 md:mb-5 leading-[1.1] tracking-tight px-2">
            Discover unique spaces for your <br className="hidden md:block" />
            <span className="text-brand-200 underline decoration-brand-200 underline-offset-4 md:underline-offset-6">next creative project</span>
          </h1>
          <p className="text-sm sm:text-base md:text-lg text-brand-100 mb-6 md:mb-8 max-w-xl mx-auto font-medium leading-relaxed px-4">
        ...
        <motion.div 
          ...
          className="bg-white/10 backdrop-blur-2xl p-1.5 md:p-2 rounded-[1.5rem] md:rounded-[2rem] shadow-2xl max-w-3xl mx-auto border border-white/20"
        >
          <div className="bg-white p-2.5 md:p-4 rounded-[1.25rem] md:rounded-[1.5rem] flex flex-col md:flex-row items-stretch md:items-center gap-2.5 md:gap-3">
            <div className="flex-[1.2] relative group" ref={locationInputRef}>
              <div className="absolute left-3.5 md:left-4 top-1/2 -translate-y-1/2">
                <MapPin className="w-4 h-4 text-brand-400 group-focus-within:text-brand-700 transition-colors pointer-events-none" />
              </div>
              <input 
                type="text" 
                value={locationInput}
                ...
                className="w-full pl-10 md:pl-11 pr-9 md:pr-10 py-2.5 md:py-3 bg-brand-50 border-none rounded-xl focus:ring-2 focus:ring-brand-200 transition-all text-brand-700 font-bold placeholder:text-brand-300 text-sm"
                autoComplete="off"
              />
              {locationInput && (
                <button
              ...
              {showSuggestions && (suggestionsLoading || suggestions.length > 0 || (locationInput.trim() !== '' && !suggestionsLoading)) && (
                <div className="absolute top-full left-0 right-0 mt-1 py-2 bg-white rounded-xl border border-brand-200 shadow-xl shadow-brand-700/10 overflow-hidden z-50 max-h-72 overflow-y-auto">
                  {suggestionsLoading ? (
                ...
                trigger={
                  <button className="w-full relative group cursor-pointer">
                    <div className="absolute left-3.5 md:left-4 top-1/2 -translate-y-1/2 z-10">
                      <Calendar className={`w-4 h-4 transition-colors ${selectedDate ? 'text-brand-700' : 'text-brand-400'}`} />
                    </div>
                    <div className="w-full pl-10 md:pl-11 pr-4 py-2.5 md:py-3 bg-brand-50 rounded-xl text-left transition-all border-2 border-transparent hover:border-brand-100 group-focus-within:ring-2 group-focus-within:ring-brand-200">
                      <span className={`text-sm font-bold truncate block ${selectedDate ? 'text-brand-700' : 'text-brand-300'}`}>
            ...
            <button 
              ...
              className="px-5 md:px-8 py-2.5 md:py-3 bg-brand-700 hover:bg-brand-600 text-white font-black text-sm md:text-base rounded-xl transition-all shadow-xl shadow-brand-700/20 active:scale-95 flex items-center justify-center gap-2 group cursor-pointer"
            >
        ...
        <motion.div 
          ...
          className="mt-6 md:mt-8 flex flex-wrap justify-center items-center gap-2.5 md:gap-3 text-white/90 text-[10px] md:text-xs font-black uppercase tracking-widest"
        >
          ...
          {(popularCategories.length > 0 ? popularCategories : DEFAULT_POPULAR_CATEGORIES).map((cat) => (
            <button
              ...
              className="px-3.5 md:px-5 py-1.5 md:py-2 bg-white/5 backdrop-blur-md border border-white/10 rounded-full hover:bg-brand-200 hover:text-brand-700 transition-all cursor-pointer"
            >
              {cat}
            </button>
          ))}
